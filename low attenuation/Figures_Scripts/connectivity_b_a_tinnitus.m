...
if (isloaded)
    ...
    if (isstruct(tmp))
        ...
        plot([1, SimParams.net.Outputs], f1.*[1, 1], ...
            '--', lws, lw/2, lcs, lc2);
        text(SimParams.net.Outputs + 1, f1/2, 'Dep.', ...
            'Color', lc2, ...
            'VerticalAlignment', 'top', ...
            'HorizontalAlignment', 'center', ...
            'Rotation', 90, ...
            'FontSize', zones_fs);
        text(SimParams.net.Outputs + 1, f1 + f2/2, 'Non-dep.', ...
            'Color', lc2, ...
            'VerticalAlignment', 'top', ...
            'HorizontalAlignment', 'center', ...
            'Rotation', 90, ...
...
%         plot(1:SimParams.net.Outputs, tmp, lws, lw, lcs, lc1);
%         set(gca,'XLim',o_lim,'XTick',o_tick,'YLim',y_K_lim,'YTick',y_K_tick);
        prof2 = zeros(1, 2*f1 - 1);
        for i = 1:f1
            prof2((1:f1) + f1 - i) = ...
                prof2((1:f1) + f1 - i) + SimParams.net.K(i, 1:f1);
        end
        prof2 = prof2./f1;
        prof2(f1) = NaN;
        prof1 = zeros(1, 2*f2 - 1);
        for i = 1:f2
            prof1((1:f2) + f2 - i) = ...
                prof1((1:f2) + f2 - i) + SimParams.net.K(f1 + i, f1 + (1:f2));
        end
        prof1 = prof1./f2;
        prof1(f2) = NaN;
%         f3 = min(f1, f2);
...
%             prof1((f1 - f3 + 1):(f1 + f3 - 1));
        p(2) = plot(-(f1 - 1):(f1 - 1), prof2, lws, lw, lcs, lc2);
        hold on;
        p(1) = plot(-(f2 - 1):(f2 - 1), prof1, lws, lw, lcs, lc1);
