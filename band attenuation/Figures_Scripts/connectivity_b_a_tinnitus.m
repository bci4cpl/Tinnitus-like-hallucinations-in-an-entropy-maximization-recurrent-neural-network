...
%         set(gca,'XLim',o_lim,'XTick',o_tick,'YLim',y_K_lim,'YTick',y_K_tick);
        ...
        f2 = round(SimParams.Attenuate.f_2*...
            SimParams.net.Outputs/SimParams.net.Inputs) - f1;
        f3 = SimParams.net.Outputs - f2 - f1;
        ...
        for i = 1:f3
            prof3((1:f3) + f3 - i) = ...
                prof3((1:f3) + f3 - i) + SimParams.net.K(f2 + i, f2 + (1:f3));
        end
        prof3 = prof3./f3;
        prof3(f3) = NaN;
        f4 = max(f1, f3);
        prof4 = zeros(1, 2*f4 - 1);
        prof4((f4-f1+1):(f4+f1-1)) = prof4((f4-f1+1):(f4+f1-1)) + f1.*prof1;
        prof4((f4-f3+1):(f4+f3-1)) = prof4((f4-f3+1):(f4+f3-1)) + f3.*prof3;
        prof4 = prof4./(f1 + f3);
