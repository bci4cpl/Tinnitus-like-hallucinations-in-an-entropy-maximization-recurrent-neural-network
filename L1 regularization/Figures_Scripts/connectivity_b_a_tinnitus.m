...
%     SetGraphDisplay(x_title, y_title, 'B');
    ...
    % Recurrent connections matrix distribution before tinnitus
    subplot(3,2,5)
    histogram(SimParams.net.K(:), 'BinWidth', 0.2, 'FaceColor', lc3);
    ax = gca;
    ax.YScale = 'log';
    ax.YLim = [0.1, Inf];
    ax.YTick = 10.^(0:2:floor(log10(numel(SimParams.net.K(:)))));
    ax.XLim = [-4, 4];
    SetGraphDisplay(y_title, '\# of Connections', 'C');
...
% % %         lx.YColor = 'k';
% % %         axes(ax);
        leg = legend(p, {'Non-deprived', 'Deprived'}, ...
            'Interpreter', 'latex', 'Location', 'north');
        leg.EdgeColor = 'k';
        SetGraphDisplay(x_title, ' ', 'E');

        % Recurrent connections matrix distribution after tinnitus
        subplot(3,2,6)
        histogram(SimParams.net.K(:), 'BinWidth', 0.2, 'FaceColor', lc3);
        ax = gca;
        ax.YScale = 'log';
